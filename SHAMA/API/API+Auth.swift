...
class API_Auth: NSObject {
    ...
    class func register(name: String,email: String, password: String, phone:String, completion: @escaping (_ error: Error?, _ success: Bool, _ data: String?, _ status: Bool?, _ message: String?)->Void) {
        ...
        Alamofire.request(url, method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil) .responseJSON { response in
            ...
            case .success(let value):
                ...
                if let status = json["status"].bool {
                    if status == true {
                        if let token = json["data"]["user_token "].string, let name = json["data"]["name"].string, let email = json["data"]["email"].string, let phone = json["data"]["phone"].string, let message = json["error"].string  {
